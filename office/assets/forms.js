...
(function () {
  ...
  // =====================================================================
  // QUOTE / ESTIMATE — price proposal (one-page, no job sheet)
  // =====================================================================
  const quote = {
    type: "quote",
  ...
  // =====================================================================
  // QUICK-PICK PRESETS — built from ~1 year of real SIAM AUTOWORKS jobs
  // =====================================================================
  const PRESETS = {
    // Tap to add a line item (qty defaults to 1, price pre-filled, editable after)
    lineItems: [
      { group: "Service", items: [
        ...
        { label: "Diagnostic & fitting labor", desc: "Diagnostic & fitting labor cost", unit: 50 },
        { label: "Travel fee", desc: "Travel fee", unit: 150 },
      ]},
      { group: "Filters", items: [
        ...
        { label: "Ryco oil filter", desc: "Ryco oil filter", unit: 25 },
      ]},
      ...
      { group: "Spark plugs", items: [
        { label: "Iridium spark plugs", desc: "Spark plugs (iridium spark plugs)", unit: 47 },
        { label: "NGK platinum spark plug", desc: "NGK platinum spark plug", unit: 49.9 },
        { label: "Spark plugs", desc: "Spark plugs", unit: 42 },
      ]},
      { group: "Brakes", items: [
        { label: "Front brake pads (ceramic)", desc: "Front brake pads (ceramic)", unit: 105 },
        { label: "Front pair ceramic pads", desc: "Front (pair) ceramic brake pads", unit: 150 },
        { label: "Rear pair ceramic pads", desc: "Rear (pair) ceramic brake pads", unit: 100 },
      ...
      { group: "Engine parts", items: [
        ...
        { label: "Rocker cover gasket", desc: "Rocker cover gasket", unit: 115 },
        { label: "Rocker cover assembly", desc: "Rocker cover assembly", unit: 295 },
      ]},
    ],

    // Tap to insert canned work/diagnosis text (matches your real wording)
    work: {
      service_diag: { label: "Service (full)", text:
        "Perform service as per manufacturer handbook. Check lights, wipers, washers and all under bonnet fluids. Check belts and hoses. Raised vehicle on jackstand and performed vehicle inspection. Check steering, brakes, suspension, exhaust and carry out a complete underside inspection of vehicle. Change engine oil and oil filter. Liaised with customer on any recommendations and/or repairs. Report and road test and check drive ability." },
      oil_change: { label: "Oil & filter change", text:
        "Change engine oil and oil filter. Drained engine oil and removed oil filter, replaced washer(s), replaced oil filter, refilled with new oil, rechecked level and cleaned area. -- ok" },
      battery_test: { label: "Battery smart test", text:
        "Battery smart test. Connected battery smart tester, tested battery - report given." },
      air_filter: { label: "Air filter", text:
        "Engine air filter replacement. Removed the attachments and replaced the engine air filter, started the engine and confirmed normal operation. -- ok" },
      cabin_filter: { label: "Cabin filter", text:
        "Cabin filter replacement. Removed the attachments and replaced the cabin filter, started the engine and confirmed normal operation. -- ok" },
      spark_plugs: { label: "Spark plugs", text:
        "Removed ignition coils to access spark plugs. Removed and replaced all spark plugs. Installed spark plugs and torqued to specification. Started engine and checked for misfire/rough running. Confirmed normal operation. -- ok" },
      battery_replace: { label: "Battery replacement", text:
        "Vehicle presented with a flat battery (no start / low voltage). Battery condition tested and confirmed failed/insufficient charge to start vehicle. Charging system visually inspected and connections checked for looseness/corrosion. Replacement battery required. Removed and replaced battery. Cleaned and secured battery terminals and clamps. -- ok" },

      // ---- NOTES / RECOMMENDATIONS blocks (from your real notes) ----
      note_std: { label: "Standard checks (full)", text:
        "Battery smart test - see attached report.\nChecked tyres. Tread depth remaining: front left ___mm right ___mm, rear left ___mm right ___mm.\nApprox front brakes ___mm remaining, rear brakes ___mm (visual inspection only, most worn pad reported).\nTyre pressure: front left ___ PSI right ___ PSI, rear left ___ PSI right ___ PSI.\nLights check - all lights working as at the time of this service.\nAir condition vent check only – ok.\nService book - completed and placed on passenger seat." },
      ...
      note_pressure: { label: "Tyre pressure", text:
        "Tyre pressure: front left ___ PSI right ___ PSI, rear left ___ PSI right ___ PSI." },
      note_lights: { label: "Lights", text:
        "Lights check - all lights working as at the time of this service." },
      note_battery: { label: "Battery report", text:
        "Battery smart test - see attached report." },
      note_aircon: { label: "Air-con vent", text:
        "Air condition vent check only – ok." },
      note_book: { label: "Service book", text:
        "Service book - completed and placed on passenger seat." },
      note_lastreplaced: { label: "Last replaced log", text:
        "Last replaced:\nEngine oil filter ___/___/______ @___km" },
    },
  };

  // Attach quick work-block chips to the relevant textareas
  const NOTE_KEYS = ["note_std","note_tyres","note_brakes","note_pressure","note_lights","note_battery","note_aircon","note_book","note_lastreplaced"];
  repair.sections.forEach(s => s.fields.forEach(f => {
    if (f.k === "diagnosis") f.quick = ["service_diag"];
    if (f.k === "work_performed") f.quick = ["oil_change","air_filter","cabin_filter","spark_plugs","battery_test","battery_replace"];
    if (f.k === "notes") f.quick = NOTE_KEYS;
  }));
  ppi.sections.forEach(s => s.fields.forEach(f => {
    if (f.k === "notes") f.quick = NOTE_KEYS;
  }));

  window.SIAM_FORMS = { repair, ppi, quote, checklistItems, PRESETS };
})();
