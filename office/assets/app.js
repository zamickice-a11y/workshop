...
(function () {
  ...
  let state = { view: "loading", jobs: [], expenses: [], bookings: [], customSnippets: [], filter: "all", search: "", moneyPeriod: "month", bookingsFilter: "new", current: null,
    entity: "ptyltd", personalUnlocked: false, pinHash: null, pinRowId: null };

  // ---------- business entity (Pty Ltd  vs  Personal/Sole trader) ----------
  // Same database, one app. Records are tagged with `entity`; the personal
  // (sole trader) side is protected by a 6-digit PIN so others can't peek.
  function bizPty()  { return (window.SIAM_CONFIG && window.SIAM_CONFIG.BUSINESS) || {}; }
  function bizSole() { return (window.SIAM_CONFIG && window.SIAM_CONFIG.BUSINESS_SOLE) || bizPty(); }
  function activeBusiness() { return state.entity === "sole" ? bizSole() : bizPty(); }
  function setEntity(e) {
    state.entity = e;
    if (window.SIAM_CONFIG) window.SIAM_CONFIG.ACTIVE_BUSINESS = activeBusiness();
  }
  // entity-filtered views (legacy rows with no entity count as Pty Ltd)
  function curJobs()     { return (state.jobs || []).filter(j => (j.entity || "ptyltd") === state.entity); }
  function curExpenses() { return (state.expenses || []).filter(x => (x.entity || "ptyltd") === state.entity); }
  function entityLabel() { return state.entity === "sole" ? "Personal ●" : "Pty Ltd"; }
  ...
  async function loadPin() {
    const { data } = await sb.from("presets").select("*").eq("kind", "pin").limit(1);
    if (data && data[0]) { state.pinHash = data[0].data && data[0].data.hash; state.pinRowId = data[0].id; }
    else { state.pinHash = null; state.pinRowId = null; }
  }
  async function savePin(hash) {
    if (state.pinRowId) {
      await sb.from("presets").update({ data: { hash } }).eq("id", state.pinRowId);
    } else {
      const res = await sb.from("presets").insert({ user_id: session.user.id, kind: "pin", label: "personal", data: { hash } }).select().single();
      if (res.data) state.pinRowId = res.data.id;
    }
    state.pinHash = hash;
  }
  async function toggleEntity() {
    if (state.entity === "sole") { setEntity("ptyltd"); openDashboard(); return; }
    if (state.personalUnlocked) { setEntity("sole"); openDashboard(); return; }
    openPinModal();
  }
  function openPinModal() {
    const setting = !state.pinHash;   // no PIN saved yet → set-a-PIN mode
    const overlay = el(`<div class="login-wrap" style="position:fixed;inset:0;z-index:340;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;padding:20px">
      <div class="login-card" style="max-width:360px;text-align:left">
        <h2 style="margin:0 0 6px;text-align:center">${setting ? "Set a personal PIN" : "Enter personal PIN"}</h2>
        <p class="muted" style="font-size:12px;margin-top:0;text-align:center">${setting ? "Choose a 6-digit PIN to lock your personal (sole trader) section." : "This section is private."}</p>
        <input id="pin1" type="password" inputmode="numeric" maxlength="6" placeholder="● ● ● ● ● ●" style="text-align:center;letter-spacing:8px;font-size:20px"/>
        ${setting ? '<input id="pin2" type="password" inputmode="numeric" maxlength="6" placeholder="Confirm PIN" style="text-align:center;letter-spacing:8px;font-size:20px;margin-top:8px"/>' : ''}
        <div id="pinErr" style="color:var(--bad);font-size:12px;margin-top:8px;min-height:16px;text-align:center"></div>
        <div class="row" style="gap:10px;margin-top:6px">
          <button class="btn" id="pinOk" style="flex:1">${setting ? "Set & enter" : "Unlock"}</button>
    ...
    setTimeout(() => $("#pin1", overlay).focus(), 50);
    const unlock = () => { state.personalUnlocked = true; close(); setEntity("sole"); openDashboard(); toast("Personal section unlocked", "ok"); };
    $("#pinOk", overlay).onclick = async () => {
      const err = $("#pinErr", overlay);
      const p1 = $("#pin1", overlay).value.trim();
      if (!/^\d{6}$/.test(p1)) { err.textContent = "PIN must be exactly 6 digits."; return; }
      if (setting) {
        const p2 = $("#pin2", overlay).value.trim();
        if (p1 !== p2) { err.textContent = "PINs don't match."; return; }
        await savePin(await sha256(p1)); unlock();
      } else {
        if (await sha256(p1) === state.pinHash) unlock();
        else { err.textContent = "Wrong PIN."; $("#pin1", overlay).value = ""; $("#pin1", overlay).focus(); }
      }
    };
  }
  ...
  function boot() {
    ...
    setEntity("ptyltd");   // default to Pty Ltd; sets ACTIVE_BUSINESS for documents
  ...
  async function autoRefresh(force) {
    ...
    try {
      ...
      if (state.view === "dashboard") {
        ...
        renderMoneyBar(); if (state.entity === "ptyltd") { renderBookingsBar(); renderScheduleBar(); }   // loadJobs already re-renders the list
      ...
      if (state.entity === "ptyltd" && nowNew > prevNew) {
  ...
  function topbar() {
    return `<div class="topbar">
      <img src="assets/logo.png" alt="logo"/>
      <span class="sp"></span>
      <button class="btn ${state.entity==='sole'?'':'btn-ghost'} btn-sm" id="entityBtn" title="Switch business">${entityLabel()}</button>
  ...
  function wireTop() {
    const b = $("#logoutBtn"); if (b) b.onclick = async () => { await sb.auth.signOut(); };
    const e = $("#entityBtn"); if (e) e.onclick = toggleEntity;
  }
  ...
  function periodTotals(key) {
    ...
    curExpenses().forEach(e => { if (inRange(new Date(e.date || e.created_at), r)) parts += expenseSigned(e); });
  ...
  async function openDashboard() {
    ...
    const isPty = state.entity === "ptyltd";
    app.innerHTML = topbar() + `<div class="wrap">
      ${state.entity==='sole' ? `<div class="card" style="margin-bottom:12px;border-color:var(--brand);background:var(--panel2)"><b>Personal — Sole Trader</b> <span class="muted" style="font-size:12px">· ${escapeHtml((activeBusiness().name)||'')} · ABN ${escapeHtml((activeBusiness().abn)||'')}</span></div>` : ``}
      ...
      ${isPty ? `<div class="card" id="bookingsBar" style="margin-bottom:12px;cursor:pointer">
    ...
    if ($("#bookingsBar")) $("#bookingsBar").onclick = openBookings;
    if ($("#scheduleBar")) $("#scheduleBar").onclick = openSchedule;
    ...
    await Promise.all([loadJobs(), loadExpenses(), loadBookings(), loadCustomSnippets(), loadPin()]);
    renderMoneyBar(); if (isPty) { renderBookingsBar(); renderScheduleBar(); }
  }
  ...
  function renderList() {
    let list = curJobs().filter(j => state.filter === "all" || j.doc_type === state.filter);
  ...
  function nextJobNo() {
    ...
    const todayCount = curJobs().filter(j => (j.job_no||"").startsWith(prefix)).length;
  ...
  async function saveJob(form, asNew) {
    ...
    const row = {
      ...
      total: totals.total, data: d, user_id: session.user.id, entity: state.entity,
    };
  ...
  function renderExpenseList() {
    ...
    const list = curExpenses().filter(e => inRange(new Date(e.date || e.created_at), r));
  ...
  function openCashSale() {
    ...
    $("#c_save", overlay).onclick = async () => {
      ...
      const row = { doc_type:"repair", job_no:data.job_no, customer:desc||"Cash sale",
        total:amount, job_date:date, status:"paid", data, user_id:session.user.id, entity:state.entity };
  ...
  function exportFinanceCSV() {
    ...
    rows.push(["","","","","Business:", (activeBusiness().name) || ""]);
    rows.push(["","","","","ABN:", (activeBusiness().abn) || ""]);
    rows.push(["","","","","Entity:", state.entity === "sole" ? "Sole trader" : "Pty Ltd"]);
    rows.push([]);
    ...
    const fname = `siam-finance-${state.entity==='sole'?'sole-':''}${slug}-${new Date().toISOString().slice(0,10)}.csv`;
