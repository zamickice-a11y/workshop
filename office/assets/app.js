...
(function () {
  ...
  function periodTotals(key) {
    ...
    let income = 0, jobsN = 0, parts = 0;
    (state.jobs || []).forEach(j => { if (inRange(jobDate(j), r)) { income += Number(j.total || 0); jobsN++; } });
    (state.expenses || []).forEach(e => { if (inRange(new Date(e.date || e.created_at), r)) parts += expenseSigned(e); });
    return { income, parts, net: income - parts, jobsN };
  }
    ...
    lc.innerHTML = `<div class="joblist">` + list.map(j => `
      <div class="jobitem" data-id="${j.id}">
        ...
        <div class="main">
          ...
          <div class="s">${escapeHtml(j.job_no || "")} · ${escapeHtml(j.rego || "")} · ${jobDate(j).toLocaleDateString("en-AU")}</div>
        </div>
  ...
  function renderMoneySummary() {
    ...
    box.innerHTML = `<div class="grid2" style="margin-top:14px">
      <div class="card"><div class="muted" style="font-size:12px">Income (${t.jobsN} jobs)</div>
      ...
      <div class="card" style="grid-column:1/-1"><div class="muted" style="font-size:12px">Net (income − parts)</div>
        ...
        <div class="muted" style="font-size:11px;margin-top:4px">Note: net before your own time/labour costs, fuel, tools, etc. For rough planning only — confirm figures with your accountant for ATO.</div></div>
    </div>`;
  }
  ...
  function openCashSale() {
    ...
    $("#c_save", overlay).onclick = async () => {
      ...
      const row = { doc_type:"repair", job_no:data.job_no, customer:desc||"Cash sale",
        total:amount, job_date:date, data, user_id:session.user.id };
