...
(function () {
  ...
  function bookingsOnDate(dateStr, excludeId){
    return (state.bookings||[])
      .filter(b => b.scheduled_date===dateStr && b.scheduled_time && b.id!==excludeId && b.status!=="archived")
      .map(b => { const start=t2m(b.scheduled_time), dur=Number(b.duration_min||DEFAULT_DURATION); return { id:b.id, name:b.name, vehicle:b.vehicle, start, end:start+dur }; })
      .sort((a,b)=>a.start-b.start);
  }
  function freeGaps(busy, startMin, endMin){
    const gaps=[]; let cur=startMin;
    busy.slice().sort((a,b)=>a.start-b.start).forEach(b=>{
      if(b.start>cur) gaps.push({start:cur, end:Math.min(b.start,endMin)});
      cur=Math.max(cur,b.end);
    });
    if(cur<endMin) gaps.push({start:cur, end:endMin});
  ...
  function openScheduleModal(b) {
    ...
    const overlay = el(`<div class="login-wrap" style="position:fixed;inset:0;z-index:330;background:rgba(0,0,0,.6);overflow:auto;padding:20px 12px">
      <div class="login-card" style="max-width:520px;text-align:left;margin:auto">
        ...
        <div class="field"><label>Duration (minutes)</label><input id="sc_dur" type="number" step="15" min="15" value="${Number(b.duration_min||DEFAULT_DURATION)}"/></div>
    ...
    function renderConflict() {
      ...
      const dur = Number($("#sc_dur", overlay).value || DEFAULT_DURATION);
      if (!time) { box.innerHTML = ""; box.dataset.clash=""; return; }
      const start = t2m(time), end = start + dur;
      const clash = bookingsOnDate(date, b.id).find(d => start < d.end && end > d.start);
      if (clash) {
        box.innerHTML = `<div style="background:rgba(255,69,58,.12);border:1px solid var(--bad);border-radius:9px;padding:10px;font-size:13px;color:var(--bad)">⚠ Clashes with <b>${escapeHtml(clash.name||"another job")}</b> (${fmtTime12(m2t(clash.start))}–${fmtTime12(m2t(clash.end))}). Pick another time.</div>`;
        box.dataset.clash = "1";
      } else {
        box.innerHTML = `<div style="background:rgba(52,199,89,.12);border:1px solid var(--ok);border-radius:9px;padding:10px;font-size:13px;color:var(--ok)">✓ ${fmtTime12(time)}–${fmtTime12(m2t(end))} is free</div>`;
        box.dataset.clash = "";
      }
    }
    function renderDay() {
      date = $("#sc_date", overlay).value;
      const dayB = bookingsOnDate(date, b.id);
      ...
      else html += dayB.map(d => `<div style="font-size:13px;margin:3px 0">${fmtTime12(m2t(d.start))}–${fmtTime12(m2t(d.end))} · <b>${escapeHtml(d.name||"(job)")}</b>${d.vehicle?(" · "+escapeHtml(d.vehicle)):""}</div>`).join("");
      const gaps = freeGaps(dayB, t2m(WORK.start), t2m(WORK.end));
      ...
      else html += `<div class="row" style="gap:6px;flex-wrap:wrap">` + gaps.map(g => `<button type="button" class="btn btn-ghost btn-sm" data-slot="${m2t(g.start)}">${fmtTime12(m2t(g.start))}–${fmtTime12(m2t(g.end))}</button>`).join("") + `</div>`;
      html += `</div>`;
    ...
    $("#sc_save", overlay).onclick = async () => {
      ...
      const { error } = await sb.from("bookings").update({
        scheduled_date: date, scheduled_time: time, duration_min: dur,
  ...
  function renderScheduleList() {
    ...
    const sched = (state.bookings||[]).filter(b => b.scheduled_date && b.status!=="archived");
    ...
    const byDate = {};
    sched.forEach(b => { (byDate[b.scheduled_date] = byDate[b.scheduled_date] || []).push(b); });
    const today = new Date().toISOString().slice(0,10);
    const tomorrow = new Date(Date.now()+86400000).toISOString().slice(0,10);
    const dayLabel = (d) => d===today ? "Today" : d===tomorrow ? "Tomorrow" : new Date(d).toLocaleDateString("en-AU",{weekday:"long",day:"numeric",month:"short"});
    let html = "";
    Object.keys(byDate).sort().forEach(d => {
      const isPast = d < today;
      html += `<div style="margin:14px 0 6px;font-weight:800;color:${isPast?'var(--muted)':'var(--brand)'}">${dayLabel(d)}${isPast?' (past)':''} <span class="muted" style="font-weight:400;font-size:12px">· ${new Date(d).toLocaleDateString("en-AU")}</span></div>`;
      html += `<div class="joblist">` + byDate[d].sort((a,b)=>(a.scheduled_time||"").localeCompare(b.scheduled_time||"")).map(b => {
        const dur = Number(b.duration_min||DEFAULT_DURATION);
        const endT = b.scheduled_time ? m2t(t2m(b.scheduled_time)+dur) : "";
        return `<div class="jobitem" data-bid="${b.id}" style="${isPast?'opacity:.55':''}">
          <span class="tag" style="background:var(--panel2);color:var(--text);min-width:78px;text-align:center">${fmtTime12(b.scheduled_time)}</span>
          <div class="main">
            <div class="t">${escapeHtml(b.name||"(no name)")}${b.vehicle?(" — "+escapeHtml(b.vehicle)):""}</div>
            <div class="s">${b.service_type?escapeHtml(b.service_type)+" · ":""}${endT?("til "+fmtTime12(endT)):""}${b.location?(" · "+escapeHtml(b.location)):""}${b.phone?(" · "+escapeHtml(b.phone)):""}</div>
          </div>
